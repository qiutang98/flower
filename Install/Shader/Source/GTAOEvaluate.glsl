...
vec3 getViewPos(vec2 uv)
{
    ...
    return getViewPos(uv, deviceZ, viewData);
}
...
void main()
{
    ...
    const vec3 viewSpaceNormal = normalize((viewData.camView * vec4(worldNormal, 0.0)).rgb);
    ...
    const float invTanHalfFovy = 1.0f / tan(viewData.camInfo.x * 0.5f);
