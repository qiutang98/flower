...
{
    return vec3(texture(sampler2D(inPrevGbufferB, linearClampBorder0000Sampler), uv).rgb);
}
...
{
    float luma = luminanceSSGI(val.xyz);
...
vec2 getHitPositionReprojection(ivec2 dispatchThreadId, vec2 uv, float reflectedRayLength) 
{
    ...
    vec3 viewSpaceRay = getViewPos(uv, z, viewData);
    ...
    vec3 worldHitPosition = (viewData.camInvertView * vec4(viewSpaceRay, 1.0)).xyz; 
    
    // Project to prev frame position.
    vec3 prevHitPosition = projectPos(worldHitPosition, viewData.camViewProjPrev);
    ...
    out vec3 reprojection) 
{
    ...
    float historyLinearDepth;
    {
        ...
        ) 
        {
            ...
            float hitHistoryLinearDepth = linearizeDepthPrev(hitHistoryDepth, viewData);
        ...
        else 
        {
            ...
            if (dot(surfaceHistoryDiff, surfaceHistoryDiff) < kReprojectSurfaceDiscardVarianceWeight * length(localNeighborhood.variance)) 
            {
                ...
                float surfaceHistoryLinearDepth = linearizeDepthPrev(surfaceHistoryDepth, viewData);
    ...
    float linearDepth = linearizeDepth(depth, viewData);
    ...
    if (disocclusionFactor < kDisocclusionThreshold) 
    {
        ...
        for (int y = -kSearchRadius; y <= kSearchRadius; y++) 
        {
            for (int x = -kSearchRadius; x <= kSearchRadius; x++) 
            {
                ...
                float historyLinearDepth = linearizeDepthPrev(historyDepth, viewData);
    ...
    if (disocclusionFactor < kDisocclusionThreshold)
    {
        ...
        float depth00 = linearizeDepthPrev(loadDepthHistory(reprojectTexelCoords + ivec2(0, 0)), viewData);
        float depth10 = linearizeDepthPrev(loadDepthHistory(reprojectTexelCoords + ivec2(1, 0)), viewData);
        float depth01 = linearizeDepthPrev(loadDepthHistory(reprojectTexelCoords + ivec2(0, 1)), viewData);
        float depth11 = linearizeDepthPrev(loadDepthHistory(reprojectTexelCoords + ivec2(1, 1)), viewData);
...
void reproject(ivec2 dispatchThreadId, ivec2 groupThreadId, uvec2 screenSize, float temporalStabilityFactor, int maxSamples) 
{
...

    {
        ...
        if (reprojectionUV.x > 0.0 && reprojectionUV.y > 0.0 && reprojectionUV.x < 1.0 && reprojectionUV.y < 1.0) 
        {
            ...
            // Config sample nums.
            float sMaxSamples = max(8.0, float(maxSamples));
...
void main()
{
    ...
    reproject(ivec2(rayCoord), ivec2(groupThreadId), screenSize, kTemporalStableReprojectFactor, kTemporalPeriod);
}