...
void main()
{
    ...
    // Main cascsade shadow compute.
    {
        ...
        shadowResult = shadowPcf(inSDSMShadowDepth, light, shadowPosOnAltas, vec2(shadowTexelSize), activeCascadeId, perCascadeOffsetUV, vec2(workPos));
    }
    ...
    if(cascadeFadeEdge > 0.0f && activeCascadeId < light.cascadeCount - 1)
    {
        ...
        float lerpShadowValue = shadowPcf(inSDSMShadowDepth, light, lerpShadowProjPos.xyz, vec2(shadowTexelSize),  lerpCascadeId, perCascadeOffsetUV, vec2(workPos));
