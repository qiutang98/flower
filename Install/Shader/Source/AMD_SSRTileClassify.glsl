...
void classifyTiles(uvec2 dispatchThreadId, uvec2 groupThreadId, float roughness)
{
    ...
    const bool bCanReflective = isShadingModelValid(texelFetch(inGbufferA, ivec2(dispatchThreadId), 0).a);
    const bool bGlossyReflection = isGlossyReflection(roughness);
    ...
    bool bNeedRay = bAllInScreen && bCanReflective && bGlossyReflection;
    ...
    if(bGlossyReflection && bCanReflective)
    {
...
void main()
{
    ...
    float perceptualRoughness = max(texelFetch(inGbufferS, workPos, 0).g, 0.003); // Slight offset roughness on ssr evaluate.
    classifyTiles(dispatchId, groupThreadId, perceptualRoughness);

    // Also store roughness.
    imageStore(SSRExtractRoughness, workPos, vec4(perceptualRoughness, 0.0f, 0.0f, 0.0f));
}