...
namespace engine
{
	...
	void FSR2Context::draw(VkCommandBuffer commandBuffer, const FfxUpscaleSetup& cameraSetup)
	{
		...
		if ((config.reactiveMaskMode == EReactiveMaskMode::On))
		{
		...
		GPUTimestamps* timer)
	{
		...
		// Config setup.
		auto& config = fsr2->config;
		{
			// Basic config.
			config.jitterX = perframe.jitterData.x;
			config.jitterY = perframe.jitterData.y;
			config.renderWidth = perframe.renderWidth;
			config.renderHeight = perframe.renderHeight;

			config.nearPlane = perframe.camInfo.z;  // We revert z.
			config.farPlane = perframe.camInfo.w;
			config.fovV = perframe.camInfo.x;
			...
			config.bCameraReset = bForceCameraCut || (perframe.bCameraCut != 0);
			...
			config.lodTextureBasicBias = perframe.basicTextureLODBias;
		}

		// ScopePerframeMarker marker(cmd, "FSR2.1.1", { 1.0f, 0.0f, 0.0f, 1.0f }, timer);
		{
			//////////////////////////////////////////////////////////////////
			...
			upscaleSetup.unresolvedColorResourceView = hdrSceneColor.getOrCreateView(buildBasicImageSubresource()).view;
			upscaleSetup.motionvectorResource = inGBuffers->gbufferV;
			upscaleSetup.motionvectorResourceView = gbufferV.getOrCreateView(buildBasicImageSubresource()).view;
			upscaleSetup.depthbufferResource = inGBuffers->depthTexture;
			upscaleSetup.depthbufferResourceView = sceneDepthZ.getOrCreateView(RHIDefaultImageSubresourceRange(VK_IMAGE_ASPECT_DEPTH_BIT)).view;
			upscaleSetup.resolvedColorResource = inGBuffers->hdrSceneColorUpscale;
			upscaleSetup.resolvedColorResourceView = displayOut.getOrCreateView(buildBasicImageSubresource()).view;

			upscaleSetup.reactiveMapResource = inGBuffers->gbufferUpscaleReactive;
			upscaleSetup.reactiveMapResourceView = inGBuffers->gbufferUpscaleReactive->getImage().getOrCreateView(buildBasicImageSubresource()).view;

			upscaleSetup.transparencyAndCompositionResource = inGBuffers->gbufferUpscaleTranslucencyAndComposition;
			upscaleSetup.transparencyAndCompositionResourceView = inGBuffers->gbufferUpscaleTranslucencyAndComposition->getImage().getOrCreateView(buildBasicImageSubresource()).view;
			...
			// FSR2 upscale.
			{
				...
				fsr2->draw(cmd, upscaleSetup);
			}
		}

		if (timer)
		{
			timer->getTimeStamp(cmd, "FSR2.1.1");
		}
	}
