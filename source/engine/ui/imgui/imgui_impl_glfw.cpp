...
#pragma region engine
    glfwSetWindowIcon(vd->Window, 1, &engine::Engine::get()->getGLFWWindows()->getData().icon);
#pragma endregion
}
...
#pragma region engine
    const std::string titleName = engine::Engine::get()->getGLFWWindows()->getName();
    glfwSetWindowTitle(vd->Window, titleName.c_str());
#pragma endregion
}
