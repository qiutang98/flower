...
namespace engine
{
	static AutoCVarFloat cVarUIFontSize("r.ImGui.FontSize", "ImGui font size.", "ImGui", 16.0f, CVarFlags::ReadOnly);
	static AutoCVarFloat cVarUIIconSize("r.ImGui.IconSize", "ImGui icon size.", "ImGui", 13.0f, CVarFlags::ReadOnly);
	...
	void styleProfessionalDark()
	{
		...
		colors[ImGuiCol_BorderShadow] = ImVec4(0.1f, 0.1f, 0.0f, 0.39f);
		{
			...
			style.Colors[ImGuiCol_SliderGrab] = ImVec4(0.8f, 0.8f, 0.80f, 1.00f);
	...
	void ImguiManager::init(VulkanContext* context)
	{
		m_context = context;
		...
		iniFilePath << Framework::get()->getConfig().configFolder << "/" << Framework::get()->getConfig().appName << "-imgui.ini";
		...
		// Descriptor pool prepare.
		{
			...
			RHICheck(vkCreateDescriptorPool(m_context->getDevice(), &pool_info, nullptr, &m_resources.descriptorPool));
		}

		renderpassBuild();

		// register swapchain rebuild functions.
		m_beforeSwapChainRebuildHandle = m_context->onBeforeSwapchainRecreate.addLambda([&]() { renderpassRelease(false); });
		m_afterSwapChainRebuildHandle = m_context->onAfterSwapchainRecreate.addLambda([&]() { renderpassBuild(); });

		// init vulkan resource.
		ImGui_ImplVulkan_InitInfo vkInitInfo{ };
		setupVulkanInitInfo(&vkInitInfo, m_resources.descriptorPool, m_context);
		...
		// upload font texture to gpu.
		{
			...
			RHICheck(vkResetCommandPool(m_context->getDevice(), commandPool, 0));
	...
	void ImguiManager::renderpassBuild()
	{
		...
		const bool bBackbufferSupportAlpha = Swapchain::isBackBufferSupportAlphaBlend(m_context);
		...
		if (this->m_resources.renderPass == VK_NULL_HANDLE)
		{
			...
			RHICheck(vkCreateRenderPass(m_context->getDevice(), &info, nullptr, &this->m_resources.renderPass));

			m_context->setResourceName(VK_OBJECT_TYPE_RENDER_PASS, (uint64_t)this->m_resources.renderPass, "ImguiRenderpass");
		}
		...
		if (shouldBlitAfterRenderUI())
		{
			...
			info.extent.width = m_context->getSwapchain().getExtent().width;
			info.extent.height = m_context->getSwapchain().getExtent().height;
			info.extent.depth = 1;
			info.mipLevels = 1;
			...
			m_drawUIImages = std::make_unique<VulkanImage>(m_context, "DrawUIImage", info, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
			LOG_TRACE("Backbuffer format not support alpha blend, so we create new image to draw and blit ui.");
		}
		else
		{
			LOG_TRACE("We just draw ui in back buffer.");
		...
		// Create Framebuffer & CommandBuffer
		{
			...
			info.attachmentCount = 1;
			info.pAttachments = attachment;
			info.width = m_context->getSwapchain().getExtent().width;
			info.height = m_context->getSwapchain().getExtent().height;
			info.layers = 1;

			auto backBufferSize = m_context->getBackBufferCount();
			...
			for (uint32_t i = 0; i < backBufferSize; i++)
			{
				if (shouldBlitAfterRenderUI())
				{
					...
					attachment[0] = m_drawUIImages->getOrCreateView(buildBasicImageSubresource());
				}
				else
				{
					// Reuse back buffer image view.
					attachment[0] = m_context->getSwapchain().getImageViews().at(i);
				}

				RHICheck(vkCreateFramebuffer(m_context->getDevice(), &info, nullptr, &m_resources.framebuffers[i]));
			}
			...
			for (uint32_t i = 0; i < backBufferSize; i++)
			{
				// Command pool
				{
					...
					info.queueFamilyIndex = m_context->getGraphiscFamily();
					RHICheck(vkCreateCommandPool(m_context->getDevice(), &info, nullptr, &m_resources.commandPools[i]));
				}
				...
				// Command buffer
				{
					...
					RHICheck(vkAllocateCommandBuffers(m_context->getDevice(), &info, &m_resources.commandBuffers[i]));
				}
			}
		}
	}
	...
	void ImguiManager::renderpassRelease(bool bFullRelease)
	{
		...
		if (bFullRelease)
		{
			vkDestroyRenderPass(m_context->getDevice(), m_resources.renderPass, nullptr);
		}
		...
		for (uint32_t i = 0; i < backBufferSize; i++)
		{
			vkFreeCommandBuffers(m_context->getDevice(), m_resources.commandPools[i], 1, &m_resources.commandBuffers[i]);
			vkDestroyCommandPool(m_context->getDevice(), m_resources.commandPools[i], nullptr);
			vkDestroyFramebuffer(m_context->getDevice(), m_resources.framebuffers[i], nullptr);
		}
	...
	{
		return m_drawUIFormat != m_context->getSwapchain().getImageFormat();
	}
	...
	void ImguiManager::renderFrame(uint32_t backBufferIndex)
	{
		...
		{
			RHICheck(vkResetCommandPool(m_context->getDevice(), m_resources.commandPools[backBufferIndex], 0));
			VkCommandBufferBeginInfo info = {};
			info.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO;
			info.flags |= VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT;
			RHICheck(vkBeginCommandBuffer(m_resources.commandBuffers[backBufferIndex], &info));
			m_context->setPerfMarkerBegin(m_resources.commandBuffers[backBufferIndex], "ImGUI", { 1.0f, 1.0f, 0.0f, 1.0f });
		}
		{
			...
			info.renderArea.extent.width = m_context->getSwapchain().getExtent().width;
			info.renderArea.extent.height = m_context->getSwapchain().getExtent().height;
		...
		m_context->setPerfMarkerEnd(m_resources.commandBuffers[backBufferIndex]);
		...
		if (shouldBlitAfterRenderUI())
		{
			...
			barrier.image = m_context->getSwapchain().getImages().at(backBufferIndex);
			...
			vkCmdBlitImage(m_resources.commandBuffers[backBufferIndex],
				...
				m_context->getSwapchain().getImages().at(backBufferIndex), VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &copyRegion, VK_FILTER_NEAREST);
	...
	void ImguiManager::release()
	{
		...
		bool res0 = m_context->onBeforeSwapchainRecreate.remove(m_beforeSwapChainRebuildHandle);
		bool res1 = m_context->onAfterSwapchainRecreate.remove(m_afterSwapChainRebuildHandle);
		...
		vkDestroyDescriptorPool(m_context->getDevice(), m_resources.descriptorPool, nullptr);
