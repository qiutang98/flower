...
vec4 grid(vec3 fragPos3D) 
{
    ...
    if(xDraw || yDraw)
    {
        ...
        if(xDraw)
        {
            color.xyz = mix(color.xyz, vec3(0.93f, 0.22f, 0.31f), a.x);
        }
        if(yDraw)
        {
            color.xyz = mix(color.xyz, vec3(0.184f, 0.51f, 0.87f), a.y);
        }
    }
    ...
    vec2 derivative = fwidth(coord * 2.0);
    float minimumz = min(derivative.y, 1);
    float minimumx = min(derivative.x, 1);

    float lineColorWidth = 0.4;

    // z axis
    bool bZAxis = false;
    if(fragPos3D.x > -lineColorWidth * minimumx && fragPos3D.x < lineColorWidth * minimumx)
    {
        color.xyz = vec3(0.0);
        bZAxis = true;
        color.xyz += vec3(0.184f, 0.51f, 0.87f);
        color.a = 1.0;
    }
    // x axis
    if(fragPos3D.z > -lineColorWidth * minimumz && fragPos3D.z < lineColorWidth * minimumz)
    {
        if(bZAxis)
        {
            color.xyz += vec3(0.93f, 0.22f, 0.31f);
        }
        else
        {
            color.xyz = vec3(0.93f, 0.22f, 0.31f);
