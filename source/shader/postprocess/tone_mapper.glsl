...
vec3 lensFlare(vec2 texcoord, vec2 sunCoord, out vec3 ghostColor)
{
    ...
    float gDist = dist * 18.0 / fovFactor;
    ...
    ghostColor = frameData.sky.color * frameData.sky.intensity * vec3(gl * GLARE_BRIGHTNESS);
    vec3 lf = frameData.sky.color * frameData.sky.intensity * fl * FLARE_BRIGHTNESS;
