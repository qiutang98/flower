    ...
    in const uint  cascadeId)
{
    ...
    float p = float(cascadeId + 1) / float(frameData.sky.cacsadeConfig.cascadeCount);
    ...
    float lambda = frameData.sky.cacsadeConfig.cascadeSplitLambda;
...
void main()
{
    ...
    if(cascadeId >= frameData.sky.cacsadeConfig.cascadeCount)
    {
        return;
    }
    ...
    farPlaneLinear = min(farPlaneLinear, nearPlaneLinear + frameData.sky.cacsadeConfig.maxDrawDepthDistance);
    ...
    const vec3 lightDir = frameData.sky.direction;
    ...
    // Texel align.
    const float sMapSize = float(frameData.sky.cacsadeConfig.percascadeDimXY);
    ...
    mat4 coarseShadowMapVP = cascadeInfos[frameData.sky.cacsadeConfig.cascadeCount - 1].viewProj;
